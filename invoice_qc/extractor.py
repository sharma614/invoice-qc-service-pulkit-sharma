...
def parse_invoice_text(text: str, filename: str = None) -> Dict:
    ...
    dates = parsing.DATE_PATTERNS[0][0].findall(text) or [] # Fallback search
    # Better: finding all dates with centralized logic
    potential_dates = []
    # Simple strategy: find all strings that look like dates and try to parse
    # Reuse the DATE_RE pattern logic but via parsing.parse_date
    for match in re.finditer(r"(\d{4}-\d{2}-\d{2}|\d{2}[\./]\d{2}[\./]\d{4}|\d{1,2}\s+[A-Za-z]{3,}\s+\d{4})", text):
        parsed = parsing.parse_date(match.group(0))
        if parsed:
            potential_dates.append(parsed)
            
    if potential_dates:
        invoice_date = potential_dates[0]
        if len(potential_dates) > 1:
            due_date = potential_dates[-1]
    ...
    def find_amount_by_label(label_patterns):
        for lp in label_patterns:
            ...
            if m:
                return parsing.parse_amount(m.group(1))
        return None
    ...
    if gross_total is None:
        amounts = parsing.AMOUNT_RE.findall(text)
        if amounts:
            gross_total = parsing.parse_amount(amounts[-1])

    currency = parsing.find_currency(text)
    ...
    if header_match:
        ...
        for ln in lines:
            m = parsing.AMOUNT_RE.search(ln[::-1])
            if m:
                last_amt_match = parsing.AMOUNT_RE.findall(ln)
                if last_amt_match:
                    try:
                        amt = parsing.parse_amount(last_amt_match[-1])
...
def extract_from_dir(pdf_dir: str) -> List[models.Invoice]:
    p = Path(pdf_dir)
    invoices = []
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        ...
        save_json([i.model_dump() for i in arr], sys.argv[2])
